...
namespace LaunchPad\Components\Client\Operating_System\OS_Check {
	...
	if(!trait_exists(__NAMESPACE__ . '\\OS_Check_IsWindowsPhone')){

		// Check if the client operating system is "Windows Phone", parse it details and store they
		trait OS_Check_IsWindowsPhone {

			// Check client operating system is "Windows Phone" or not
			// @no_variable
			// @return Boolean
			public static function OS_Check_IsWindowsPhone() : Bool {
				if(Framework\Validator::Contain(self::$agent, 'Windows Phone', false, true)){
					self::$operating_system['name'] = 'Windows Phone';
					if($version = Framework\Validator::ContainRegex(self::$agent, 'Windows Phone ([\d\.]*)', null, true)){
	...
	return(\LaunchPad\Components\Client\Operating_System\OS_Check\OS_Check_IsWindowsPhone::class);
