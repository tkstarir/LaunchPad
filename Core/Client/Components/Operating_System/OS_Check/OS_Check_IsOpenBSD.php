...
namespace LaunchPad\Components\Client\Operating_System\OS_Check {
	...
	if(!trait_exists(__NAMESPACE__ . '\\OS_Check_IsOpenBSD')){

		// Check if the client operating system is "Open BSD", parse it details and store they
		trait OS_Check_IsOpenBSD {

			// Check client operating system is "Open BSD" or not
			// @no_variable
			// @return Boolean
			public static function OS_Check_IsOpenBSD() : Bool {
				if(Framework\Validator::Contain(self::$agent, 'OpenBSD', false, true)){
					self::$operating_system['name'] = 'OpenBSD';
	...
	return(\LaunchPad\Components\Client\Operating_System\OS_Check\OS_Check_IsOpenBSD::class);
