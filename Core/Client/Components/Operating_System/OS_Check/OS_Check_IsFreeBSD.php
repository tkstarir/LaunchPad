...
namespace LaunchPad\Components\Client\Operating_System\OS_Check {
	...
	if(!trait_exists(__NAMESPACE__ . '\\OS_Check_IsFreeBSD')){

		// Check if the client operating system is "Free BSD", parse it details and store they
		trait OS_Check_IsFreeBSD {

			// Check client operating system is "Free BSD" or not
			// @no_variable
			// @return Boolean
			public static function OS_Check_IsFreeBSD() : Bool {
				if(Framework\Validator::Contain(self::$agent, 'FreeBSD', false, true)){
					self::$operating_system['name'] = 'Free BSD';
	...
	return(\LaunchPad\Components\Client\Operating_System\OS_Check\OS_Check_IsFreeBSD::class);
